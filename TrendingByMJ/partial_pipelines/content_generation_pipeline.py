...
async def test_complete_replicate_pipeline_whisper():
    ...
    t1_5 = time.time()
    story_folder = Config.STORIES_DIR / sanitized_title
    ...
    with tqdm(total=1, desc="Whisper audio sync", unit="sync") as pbar:
        ...
        # Process audio for image synchronization (now async) - Use 6 images for trending topics
        synchronized_prompts = await whisper_sync.process_audio_for_image_sync(
            audio_path=audio_path,
            original_story=story_data['story'],
            num_images=6,  # Use 6 images for trending topics
    ...
    print(f"📁 Whisper sync data saved to: {Config.OUTPUT_DIR}/audio_sync_data/{sanitized_title}_whisper_sync.json")
    ...
    with tqdm(total=len(image_prompts), desc="Generating images with Schnell", unit="image") as pbar:
        result = await image_generator.generate_images_for_story(
            ...
            custom_folder_name=sanitized_title
        )
    ...
    # Show audio processing info
    print(f"\n🎵 Audio Processing:")
    print(f"   ✅ Using original TTS audio")
    print(f"   ✅ Natural pacing and timing")
    print(f"   ✅ No audio quality modifications")
    print(f"   ✅ Direct mixing with background music")
    print(f"\n💰 Cost Breakdown:")
...
async def test_complete_replicate_pipeline_whisper_with_story(story_data: dict, topic_name: str = None, logger=None):
    """Test the complete AutoTube pipeline with pre-generated story data + Replicate (Schnell) image generation + Whisper audio sync."""
    print(f"\n🚀 AutoTube Pipeline Test - Pre-generated Story + Replicate (Schnell) + Whisper Audio Sync\n" + ("="*70))
    timings = {}
    t0 = time.time()
    
    # Use provided story data instead of generating new story
    print(f"\n[STEP 1] 📝 Using Pre-generated Story...")
    t1 = time.time()
    
    # Validate the provided story data
    if not validate_story_data(story_data, "Pre-generated Story Validation", logger):
        print("\n❌ PRE-GENERATED STORY VALIDATION FAILED - STOPPING PIPELINE")
        logger.error("❌ PRE-GENERATED STORY VALIDATION FAILED - STOPPING PIPELINE")
        return False
    
    print(f"✅ Using pre-generated story: {story_data['title']}")
    print(f"✅ Hook: {story_data.get('hook', '')}")
    print(f"✅ Word count: {len(story_data.get('story', '').split())}")
    print(f"✅ Music category: {story_data.get('music_category', 'Unknown')}")
    
    timings['Story Validation'] = time.time() - t1
    print(f"⏱️ Story validation completed in {timings['Story Validation']:.2f}s")

    # Create universal sanitized title for all asset folders
    # Use topic_name if provided, otherwise use story title
    if topic_name:
        sanitized_title = sanitize_folder_name(topic_name)
        print(f"📁 Using topic-based folder name: {sanitized_title}")
    else:
        sanitized_title = sanitize_folder_name(story_data['title'])
        print(f"📁 Using story-based folder name: {sanitized_title}")
    
    # Setup logging with file if not provided
    if logger is None:
        logger = setup_logging_with_file(topic_name or story_data['title'], "content_gen")
    logger.info(f"📁 Sanitized folder name: {sanitized_title}")
    ...
    topic_txt_path = story_folder / 'topic.txt'
    with open(topic_txt_path, 'w', encoding='utf-8') as f:
        f.write(topic_name or story_data['title'])
    ...
    audio_filename = f"audio_{sanitized_title}.mp3"
    with tqdm(total=1, desc="Generating TTS audio", unit="audio") as pbar:
        # Use topic_name for folder structure, not story title
        folder_name = topic_name if topic_name else story_data['title']
        audio_path = tts_story_to_audio(story_data['story'], audio_filename, story_title=folder_name)
        pbar.update(1)
    ...
    with tqdm(total=len(image_prompts), desc="Generating images with Schnell", unit="image") as pbar:
        result = await image_generator.generate_images_for_story(
            ...
            image_prompts=image_prompts,
            quality_preset="mvp_testing",
            custom_folder_name=topic_name if topic_name else story_data['title']
        )
        pbar.update(len(image_prompts) - result.get('successful_images', 0))
    ...
    print(f"📁 Images: {Config.OUTPUT_DIR}/images/{sanitized_title}/")
    ...
    logger.info(f"🎉 Content generation pipeline completed successfully in {total_time:.2f}s")
    logger.info(f"📁 Final sanitized title: {sanitized_title}")
