...
class WhisperAudioSynchronizer:
    ...
    async def generate_enhanced_image_prompt(
        ...
        try:
            ...
            async with StoryGenerator() as generator:
                ...
                # Detect if story is about a specific person
                person_focus = self._detect_person_focus(story_context, audio_content)
                ...
                STORY CONTEXT: {story_context}
                IMAGE NUMBER: {image_number} (out of 12)
                {f"PERSON FOCUS: {person_focus}" if person_focus else ""}
                ...
                - Maximum 50 words
                {f"- FOCUS ON THE PERSON: {person_focus} - show their emotions, reactions, or actions" if person_focus else ""}
                {f"- If the person is mentioned in the audio, make them the central focus of the image" if person_focus else ""}
                {f"- Show the person's facial expressions, body language, or emotional state" if person_focus else ""}
    ...
    def _detect_person_focus(self, story_context: str, audio_content: str) -> str:
        """
        Detect if the story is about a specific person and return their name/description
        
        Args:
            story_context: The full story context
            audio_content: The current audio segment
            
        Returns:
            Person name/description if detected, empty string otherwise
        """
        try:
            # Common person indicators
            person_indicators = [
                'actor', 'actress', 'celebrity', 'star', 'performer', 'artist', 'musician',
                'politician', 'leader', 'president', 'minister', 'official', 'executive',
                'athlete', 'player', 'coach', 'sportsman', 'sportswoman',
                'author', 'writer', 'director', 'producer', 'creator',
                'scientist', 'researcher', 'doctor', 'professor', 'expert',
                'businessman', 'businesswoman', 'entrepreneur', 'CEO', 'founder'
            ]
            
            # Look for person names in the context (common patterns)
            import re
            ...
            # Check story context for person names
            for pattern in name_patterns:
                matches = re.findall(pattern, story_context)
                if matches:
                    # Return the first person name found
                    return matches[0]
            
            # Check for person indicators + names
            for indicator in person_indicators:
                if indicator.lower() in story_context.lower():
                    # Look for a name near this indicator
                    words = story_context.split()
                    for i, word in enumerate(words):
                        if indicator.lower() in word.lower():
                            # Check next few words for a potential name
                            for j in range(i+1, min(i+4, len(words))):
                                if words[j][0].isupper() and len(words[j]) > 2:
                                    return words[j]
            
            # Check audio content for person names
            for pattern in name_patterns:
                matches = re.findall(pattern, audio_content)
                if matches:
                    return matches[0]
            ...
            self.logger.warning(f"Error detecting person focus: {e}")
            return ""
            # Fallback to template-based prompt
            shot_descriptions = {
                'wide_shot': 'wide cinematic shot showing expansive view',
