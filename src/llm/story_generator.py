...
class StoryGenerator:
    ...
    async def generate_story(self, topic: str, use_interactive_prompt: bool = False) -> Dict[str, Any]:
        ...
        if word_count < 80:
            print(f"[WARNING] Story is too short! Expected 80-90 words, got {word_count}")
        elif word_count > 90:
            print(f"[WARNING] Story is too long! Expected 80-90 words, got {word_count}")
    ...
    async def _regenerate_safe_story(self, topic: str) -> Dict[str, Any]:
        ...
        try:
            ...
            The story MUST be 80-90 words for 30-second video duration.
            
            Return your response in this EXACT JSON format:
            {{
                "title": "Viral title under 60 characters (NO EMOJIS, avoid starting with 'The')",
                "story": "Full story (80-90 words) starting with a powerful hook and voice tags, flowing naturally into the narrative with cinematic pacing. Every word must serve the story - NO filler content.",
    ...
    async def generate_image_moments_and_prompts(self, story_text: str, num_images: int = 12) -> List[str]:
        ...
        split_prompt = (
            ...
            f"Given the following story, break it into EXACTLY {num_images} visually distinct, cinematic moments that would best illustrate the story as a sequence of images for a 30-second YouTube Short. "
    ...
    async def generate_story_driven_visual_sequence(self, story_text: str, num_images: int = None) -> list:
        ...
        import json
        import re
        # 1. Always use 12 images for 30-second videos
        num_images = 12
...
- Must be suitable for a 30-second video
...
Make it compelling and attention-grabbing for a 30-second video.
